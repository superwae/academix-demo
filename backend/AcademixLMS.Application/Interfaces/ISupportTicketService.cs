...
public interface ISupportTicketService
{
    ...
    Task<Result<IReadOnlyList<SupportTicketDto>>> GetAllForStaffAsync(
        string? statusFilter,
        Guid? assignedToUserId,
        bool unassignedOnly,
        CancellationToken cancellationToken = default);
    Task<Result<IReadOnlyList<SupportStaffMemberDto>>> GetSupportStaffAsync(CancellationToken cancellationToken = default);
    Task<Result<SupportTicketDetailDto>> GetByIdAsync(Guid ticketId, Guid requestingUserId, bool isStaff, CancellationToken cancellationToken = default);
